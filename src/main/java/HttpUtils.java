...
import CommonUtils.ImmutablePair;
import CommonUtils.Pair;
import ObjectMap.ObjectMapper;
...
public class HttpUtils {
    ...
    public static <Req, Resp> Resp performPostRequest(
        ...
        Pair<Integer, String> cfResponse = HttpUtils.fetchPostResponseFromServer(conn, ObjectMapper.write(request));
        conn.disconnect();

        if (cfResponse.getLeft() != 200) {
            // Non success response. Do something.
        }

        return ObjectMapper.readValue(cfResponse.getRight(), respClass);
    }
